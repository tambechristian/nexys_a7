...
//`ifdef SDCARDSPIMODE
//`include "spimode_sdcard.v" //must be commented out to avoid overwriting critical warning errors.
//`elsif SDCARDSDMODE
...
parameter CLKFREQ = 0; // 100000000; //0;
...
input[2 -1 : 0] memop;
input[(clog2(CACHESIZE*2) -2) -1 : 0] memaddr; //10 bit - 2 = 8 bits address
...
//$display("The decimal value of status is: %d", status);  //for simulation only
always @* begin
    //$display(status, ";") ; //for simulation only
    //$display("The decimal value of status is: %d", status); //for simulation only
    //$display("The decimal value of rst is: %d", rst); //for simulation only
