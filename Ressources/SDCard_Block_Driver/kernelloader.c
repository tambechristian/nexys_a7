...
//#define SDRAMBASEADDR XPAR_AXI4SDRSDRAM_0_S_AXI_MEM0_BASEADDR
#define SDCARDCNTRLRADDR XPAR_AXI4SDCARD_0_BASEADDR
//#define SDRAMBYTESIZE ((XPAR_AXI4SDRSDRAM_0_S_AXI_MEM0_HIGHADDR+1) - XPAR_AXI4SDRSDRAM_0_S_AXI_MEM0_BASEADDR)
...
uint sdcardcntrlrread (sdcardcntrlr* dev, void* ptr, uint index, uint count) {
	...
	if ((index + count) > dev->blockcount) return 0;
...
uint sdcardcntrlrcopy (sdcardcntrlr* dev, uint dstindex, uint srcindex, uint count) {
	...
	if (((dstindex + count) > dev->blockcount) || ((srcindex + count) > dev->blockcount)) return 0;
...
int main () {
	...
	while (!sdcardcntrlrinit(&dev)) {
		//xil_printf("ERROR: SDCard initialization failed; retrying ...\n\r");
	}

	if ((size = dev.blockcount/((1024*1024*1024)/512)))
		xil_printf("SDCard size: %d GBytes\n\r", size);
	else if ((size = dev.blockcount/((1024*1024)/512)))
		xil_printf("SDCard size: %d MBytes\n\r", size);
	else if ((size = dev.blockcount/(1024/512)))
		xil_printf("SDCard size: %d KBytes\n\r", size);
	else xil_printf("SDCard size: %d Bytes\n\r", dev.blockcount*512);
	...
	// uint partition1blockcount;

	// List MBR partitions.
	// xil_printf("Partition0: 0x%02X, blockoffset: %d, blockcount: %d\n\r",
		// mbr->partition[0].type, mbr->partition[0].lbabegin, mbr->partition[0].blockcount);
	// xil_printf("Partition1: 0x%02X, blockoffset: %d, blockcount: %d\n\r",
		// mbr->partition[1].type,
		// partition1lbabegin = mbr->partition[1].lbabegin,
		// partition1blockcount = mbr->partition[1].blockcount);
	// xil_printf("Partition2: 0x%02X, blockoffset: %d, blockcount: %d\n\r",
		// mbr->partition[2].type, mbr->partition[2].lbabegin, mbr->partition[2].blockcount);
	// xil_printf("Partition3: 0x%02X, blockoffset: %d, blockcount: %d\n\r",
		// mbr->partition[3].type, mbr->partition[3].lbabegin, mbr->partition[3].blockcount);

	// Hang if the partition from
	// which the kernel will be loaded
	// has a null size.
	// if (!partition1blockcount) {
	...
	// if ((partition1lbabegin + partition1blockcount) > dev.blockcount) {
	...
	// if (partition1blockcount > (SDRAMBYTESIZE/512)) {
	...
	// uint blockcountreaded = 0;

	// while (partition1blockcount - (blockcountreaded += sdcardcntrlrread(&dev,
		// (void*)SDRAMBASEADDR + (blockcountreaded*512),
			// partition1lbabegin + blockcountreaded,
				// partition1blockcount - blockcountreaded))) {
