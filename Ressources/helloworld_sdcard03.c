...
uint sdcardcntrlrinit (sdcardcntrlr* dev) {
	...
	*(volatile uint*)(dev->addr+516) = CMDRESET;
	...
	do {
		...
		//xil_printf("waiting for status being ready!! ...\n\r");
		if (n == STATUSPOWEROFF || n == STATUSERROR){
			xil_printf("Status state is %d ...\n\r", n);
			return 0;
		}

	} while (n != STATUSREADY);
	xil_printf("status returned ready!! ...\n\r");

	dev->blkcnt = *(volatile uint*)(dev->addr+516);

	//xil_printf("control init successfull!! ...\n\r");
